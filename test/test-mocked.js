...
global.fetchReal = global.fetch
global.fetchMocked = function fetchMocked (resource, options) {
...
describe('mocked', function () {
  before(function () {
    global.fetch = global.fetchMocked
  })

  after(function () {
    global.fetch = global.fetchReal
  })

  beforeEach(function () {
    this.currentTest.responseTimer = new Timer()
  })
  ...
  describe('one (basic)', function () {
    ...
    it('should execute one fetch', async function () {
      const conn = await Connection.create()
      this.test.responseTimer.add(await conn.one('req:200/moop1/500'))
      this.test.responseTimer.validate('res:200/moop1/500')
    })

    it('should execute two fetches serially', async function () {
      const conn = await Connection.create()
      this.test.responseTimer.add(await conn.one('req:200/moop1/600'))
      this.test.responseTimer.add(await conn.one('req:200/moop2/400'))

      this.test.responseTimer.validate([
        'res:200/moop1/600',
        'res:200/moop2/1000'
      ])
    })

    it('should execute two fetches serially with extra separation due to high minMsBetweenRequests', async function () {
      const conn = await Connection.create({ minMsBetweenRequests: 500 })
      this.test.responseTimer.add(await conn.one('req:200/moop1/200'))
      this.test.responseTimer.add(await conn.one('req:200/moop2/400'))
    ...
    it('should execute five fetches serially', async function () {
      const conn = await Connection.create()
      this.test.responseTimer.add(await conn.one('req:200/moop1/300'))
      this.test.responseTimer.add(await conn.one('req:200/moop2/300'))
      this.test.responseTimer.add(await conn.one('req:200/moop3/300'))
      this.test.responseTimer.add(await conn.one('req:200/moop4/200'))
      this.test.responseTimer.add(await conn.one('req:200/moop5/400'))
  ...
  describe('swarm (basic)', function () {
    ...
    it('should execute a swarm of one', async function () {
      ...
      for await (const response of conn.swarm(['req:200/moop1/300'])) {
        this.test.responseTimer.add(response)
      }
      this.test.responseTimer.validate(['res:200/moop1/300'])
    })

    it('should execute a swarm of two concurrently', async function () {
      const conn = await Connection.create({ minMsBetweenRequests: 100 })
      for await (const response of conn.swarm(['req:200/moop1/300', 'req:200/moop2/400'])) {
        this.test.responseTimer.add(response)
      }
      this.test.responseTimer.validate(['res:200/moop1/300', 'res:200/moop2/500'])
    })

    it('should execute a swarm of five semi-concurrently', async function () {
      const conn = await Connection.create({ minMsBetweenRequests: 100 })
      for await (const response of conn.swarm(['req:200/moop1/700', 'req:200/moop2/300', 'req:200/moop3/200', 'req:200/moop4/500', 'req:200/moop5/200'])) {
        this.test.responseTimer.add(response)
      }
      this.test.responseTimer.validate(['res:200/moop1/700', 'res:200/moop2/400', 'res:200/moop3/600', 'res:200/moop4/1100', 'res:200/moop5/900'])
    })

    it('should execute a swarm of six concurrently', async function () {
      const conn = await Connection.create({ minMsBetweenRequests: 100, concurrency: 6 })
      for await (const response of conn.swarm(['req:200/moop1/700', 'req:200/moop2/300', 'req:200/moop3/200', 'req:200/moop4/500', 'req:200/moop5/200', 'req:200/moop6/400'])) {
        this.test.responseTimer.add(response)
      }
      this.test.responseTimer.validate(['res:200/moop1/700', 'res:200/moop2/400', 'res:200/moop3/400', 'res:200/moop4/800', 'res:200/moop5/600', 'res:200/moop6/900'])
    })
  })

  describe('swarm (nested)', function () {
    this.timeout(10000)
    this.slow(5000)

    it('should prioritize inner loops', async function () {
      const conn = await Connection.create()
      // for await (const response of conn.swarm([]))
    })
  })
